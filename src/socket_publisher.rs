...
pub fn run<S: Sensor>(teardown: Arc<AtomicBool>, socket_path: &str, sensor_interval: u64, sensor: S, gp: Arc<Mutex<Gpio>>) -> Result<(), Error> {
    ...
    while !teardown.load(Ordering::SeqCst) {
        match listener.accept() {
            Ok((mut stream, _addr)) => {
                loop {
                    match stream.write_all(&u32_to_bytes(sensor.read(&mut gp.lock().unwrap()).unwrap())) {
